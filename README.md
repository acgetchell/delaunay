...
![3D Delaunay triangulation and convex hull generated by the quickstart notebook][readme-hero]
...
- Rust 1.97.0 or newer, pinned by `Cargo.toml` and `rust-toolchain.toml`.
...
notebook uses the opt-in `delaunay` binary as the engine, loads generated triangulation and convex
hull JSON, and writes the transparent hero preview under `target/notebooks/00_quickstart/`.
