...
- `ORIENTATION_SPEC.md` documents one invariant in detail: how cell orderings,
...
At the TDS layer, neighboring cells must induce compatible orientations on their
...
- [`Tds::is_coherently_oriented`](../src/core/tds.rs) returns `true` when all
  non-periodic adjacent cell pairs satisfy the facet-parity convention.
...
- `Tds::normalize_coherent_orientation()` computes a BFS flip assignment over
  each connected component and swaps vertex slots `0 <-> 1` for cells that must
  flip.
...
neighbor wiring, but skips combinatorial facet-parity checks when either cell in
the adjacent pair carries periodic offsets.

### Positive Geometric Cell Orientation

At the `Triangulation` layer, stored cells are promoted toward the positive
geometric orientation convention using exact orientation predicates. This is a
coordinate-dependent invariant.

Implementation entry points:

- `Triangulation::validate_geometric_cell_orientation()` evaluates each cell
...
- `Triangulation::normalize_and_promote_positive_orientation()` first runs
  ...
  performs a bounded per-cell promotion fallback for near-degenerate edge cases.
- `Triangulation::canonicalize_positive_orientation_for_cells()` is used on
  newly inserted cells so incremental insertion does not leave negative cells in
  the mutation frontier.
- `Triangulation::validate_geometric_nondegeneracy()` is stricter: it rejects
  zero-determinant cells. Explicit user-provided cell construction calls this
  after structural/topological validation.

`validate_geometric_cell_orientation()` rejects negative signs, but it does not
...
user-provided cells, while some repair paths can encounter near-degenerate
intermediate cells where coherent combinatorial orientation remains the primary
safety property.
...
return a deterministic ±1 for degenerate finite inputs. Cell orientation
...
For adjacent cells, the shared facet is represented by the vertex ordering
obtained from each cell after omitting the opposite vertex slot.
...
- `observed_odd_permutation`: whether the permutation from the first cell's
  facet ordering to the neighbor cell's facet ordering is odd.
...
For ordinary Euclidean cells, facet identities are just `VertexKey`s in
cell-local order. For periodic cells, the helper can build lifted identities
...
- `Tds::validate_coherent_orientation_for_cells(cells)`
...
cell-neighbor graph. It assigns a boolean "flip this cell" state by propagating
...
`1` in every cell assigned `true`.

`Tds::is_valid()` includes coherent orientation as a Level 2 structural check
after cell vertex keys, duplicate cells, facet sharing, and neighbor consistency
...
- Incremental cavity filling inserts new cells, then calls
  `canonicalize_positive_orientation_for_cells(&new_cells)`.
- Local insertion validation calls both
  `tds.validate_coherent_orientation_for_cells(cells)` and
  `validate_geometric_cell_orientation_for_cells(cells)`.
- Whole-triangulation topology validation runs `validate_topology_core()` and
  then `validate_geometric_cell_orientation()`.
...
- Candidate replacement cells are checked with `robust_orientation`.
- `Orientation::DEGENERATE` becomes `FlipError::DegenerateCell`.
- `Orientation::NEGATIVE` swaps two vertex slots before insertion.
- `orient_replacement_cells()` chooses replacement-cell parity from the oriented
  cavity boundary and internal replacement-cell adjacencies.
- `ReplacementOrientationPolicy::RequirePositive` validates positive geometric
  orientation for Delaunay-repair replacement cells.
...
drive repair, but replacement-cell orientation itself uses `robust_orientation`.

### Builder Paths

`src/triangulation/builder.rs` normalizes explicit and periodic construction:

- `from_vertices_and_cells(...)` accepts user-provided cell orderings, assembles
  the TDS, calls `normalize_and_promote_positive_orientation()`, validates TDS
  structure/topology, rejects geometrically degenerate cells, and then enforces
...
- Explicit user-provided cells are rejected by
  `validate_geometric_nondegeneracy()`.
- Flip replacement cells are rejected as `FlipError::DegenerateCell`.
- `validate_geometric_cell_orientation()` does not reject zero determinants by
...
- [`tests/tds_orientation.rs`](../tests/tds_orientation.rs): construction,
  coherent orientation, and tamper detection in 2D through 5D.
...
- [`tests/triangulation_builder.rs`](../tests/triangulation_builder.rs):
  explicit construction normalizes incoherent user cell orderings.
- [`tests/regressions.rs`](../tests/regressions.rs): regression coverage for the
  4D bulk-repair orientation cleanup that previously left negative cells.
...
- Full coherent-orientation validation is O(cells × D³): each cell has O(D)
  facets, each facet parity check uses O(D²) inversion counting.
- Local mutation paths validate only the cells touched by insertion or repair
...
- Debug assertions around flips compile out in release builds, but the actual
  replacement-cell orientation checks are part of correctness and remain active.
...
When adding or changing a path that creates, removes, rewires, or reorders cells:

- Preserve cell-local vertex/neighbor/periodic-offset slot alignment when
  swapping vertex slots.
- Use `robust_orientation`, not `Kernel::orientation()`, when detecting
  geometric cell sign or degeneracy.
- Normalize coherent orientation after assembling cells whose relative parity is
  not already fixed by construction.
- Validate the local mutation frontier with
  `validate_coherent_orientation_for_cells` and geometric orientation checks
...
- Use typed errors: `TdsError::OrientationViolation` for combinatorial
  ...
  cells, and `FlipError::DegenerateCell` for degenerate flip replacements.
- Add tests that tamper with cell orderings or exercise the new mutation path in
