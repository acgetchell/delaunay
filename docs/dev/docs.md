...
- `papers/*.tex` owns publication-facing mathematical exposition. The matching
  ...
  `target/papers/` with Tectonic by `just papers` or `just paper-build`.
- `papers/generated/*.png` owns stable paper figures generated by notebooks.
  Regenerate them with `just paper-figures`; ordinary notebook checks write
  only under `target/notebooks/`.
