...
## Isolated Downstream Fixtures

[`fixtures/checkpoint_no_float_roundtrip/`](./fixtures/checkpoint_no_float_roundtrip/)
is intentionally a standalone Cargo crate, not generated build output. It
verifies that an ordinary downstream application can serialize and restore a
schema-v2 checkpoint without enabling `serde_json/float_roundtrip`, while still
preserving the exact bits of difficult `f64` coordinates.

A normal integration test shares this package's Cargo dependency graph. Features
enabled by any root development or tooling dependency can therefore be unified
into that graph and make a downstream-compatibility regression pass for the
wrong reason. The fixture creates a separate resolution boundary that models an
independent consumer:

- `Cargo.toml` declares only the dependencies used by that consumer. The empty
  `[workspace]` table makes this manifest an explicit workspace root, with the
  fixture package itself as its sole implicit member. “Empty” means there are no
  additional members or workspace settings; it does not mean the package belongs
  to no workspace. Cargo stops ancestor-workspace discovery at this manifest, so
  the fixture owns its lockfile and resolution boundary even if the repository's
  workspace layout changes later.
- `Cargo.lock` is generated by Cargo but deliberately committed. The harness
...
- [`checkpoint_downstream.rs`](./checkpoint_downstream.rs) launches the fixture
  ...
  must never be committed under `tests/fixtures/`.

The nested directory alone does not isolate Cargo features. The harness's
separate `cargo run --manifest-path ...` invocation creates the independent
build graph; the fixture's `[workspace]` table makes its manifest the explicit
root of that graph.

Run the regression with:

```bash
cargo nextest run --release --profile ci --test checkpoint_downstream
```

Do not absorb this fixture into the root package or remove its nested workspace
without replacing the dependency-isolation guarantee. Doing so can make the test
susceptible to Cargo feature unification again.

The fixture is repository-only test infrastructure. Cargo excludes nested
packages from published archives, so `checkpoint_downstream.rs` recognizes the
archive-only `.cargo_vcs_info.json` marker and returns when the fixture manifest
is absent there. A missing fixture remains a test failure in a normal checkout.
This keeps `cargo package` warning-free while ensuring an extracted crate never
tries to launch a manifest that Cargo did not package.
