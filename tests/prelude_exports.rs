...
use delaunay::prelude::export::{
    ...
    VisualizationDataValidationError as ExportPreludeVisualizationDataValidationError,
    VisualizationExportError as ExportPreludeVisualizationExportError,
...
use delaunay::prelude::geometry::{
    AdaptiveKernel, ArrayConversionFailureReason, CircumcenterError, CircumcenterFailureReason,
...
use delaunay::{
    ...
    VISUALIZATION_SCHEMA as RootVisualizationSchema,
    ValidatedVisualizationData as RootValidatedVisualizationData,
    VisualizationData as RootVisualizationData,
    VisualizationDataValidationError as RootVisualizationDataValidationError,
    VisualizationExportError as RootVisualizationExportError,
...
enum RootApiExportTestError {
    ...
    #[error(transparent)]
    VisualizationExport(#[from] RootVisualizationExportError),
    #[error(transparent)]
    VisualizationValidation(#[from] RootVisualizationDataValidationError),
}
...
enum PreludeExportTestError {
    ...
    #[error(transparent)]
    VisualizationExport(#[from] ExportPreludeVisualizationExportError),
    #[error(transparent)]
    ...
    #[error(transparent)]
    ConvexHullQuery(#[from] ConvexHullQueryError),
    #[error(transparent)]
    ...
    #[error(transparent)]
    SphericalPoint(#[from] SphericalPointError),
    #[error(transparent)]
    SphericalSimplex(#[from] SphericalSimplexError),
    #[error(transparent)]
    TdsDraft(#[from] TdsDraftError),
    #[error(transparent)]
    TdsDraftInsertion(#[from] TdsDraftInsertionError),
    #[error(transparent)]
    TdsConstruction(#[from] TdsConstructionError),
    #[error(transparent)]
    GenericTriangulationBuild(#[from] GenericTriangulationBuildFailure<(), (), 2>),
    #[error(transparent)]
    Invariant(#[from] InvariantError),
    #[error("expected fixture state was missing: {context}")]
    MissingFixture { context: &'static str },
}
...
impl From<InvalidCoordinateValue> for PreludeExportTestError {
    fn from(source: InvalidCoordinateValue) -> Self {
        Self::InvalidCoordinateValue(source)
    }
}
...
fn construction_prelude_exports_builder_statistics_terminal() -> Result<(), PreludeExportTestError>
{
    ...
    let focused_error: DelaunayTriangulationConstructionErrorWithStatistics =
        DelaunayTriangulationBuilder::new(&no_vertices)
            .build_with_statistics()
            .expect_err("empty construction should fail");
...
#[test]
fn construction_prelude_covers_dedup_policy() -> Result<(), PreludeExportTestError> {
    assert_eq!(
        DedupTolerance::try_new(-1.0),
        Err(DeduplicationError::NegativeEpsilon)
    );
    let dedup_policy = DedupPolicy::epsilon(DedupTolerance::try_new(0.0)?);
...
#[test]
fn construction_prelude_covers_spherical_delaunay_exports() -> Result<(), PreludeExportTestError> {
    ...
    let triangulation: SphericalDelaunayTriangulation<2> =
        SphericalDelaunayBuilder::<2>::try_new(points)?.build()?;
...
fn root_exports_cover_flattened_public_api() -> Result<(), RootApiExportTestError> {
    ...
    let validation_result: Result<(), ValidationModuleDelaunayTriangulationValidationError> =
        dt.validate();
    validation_result?;
    let root_visualization_data: RootVisualizationData<3> = dt.to_visualization_data()?;
    assert_eq!(
        root_visualization_data.metadata.schema,
        RootVisualizationSchema
    );
    assert_eq!(
        root_visualization_data.metadata.topology_kind,
        RootVisualizationTopologyKind::Euclidean
    );
    assert_eq!(
        root_visualization_data.metadata.topology_guarantee,
        RootVisualizationTopologyGuarantee::PLManifold
    );
    let root_validated_data: RootValidatedVisualizationData<3> =
        root_visualization_data.into_validated()?;
    assert_eq!(
        root_validated_data.metadata().schema,
        RootVisualizationSchema
    );
    ...
    let incidence =
        facet_index
            ...
            .ok_or(PreludeExportTestError::MissingFixture {
                context: "fresh facet-incidence entry",
            })?;
...
fn assert_ridge_handle_exports<K>(
    ...
    let iter_ridge =
        dt.ridge_handles()
            ...
            .transpose()?
            .ok_or(PreludeExportTestError::MissingFixture {
                context: "ridge in a constructed tetrahedron",
            })?;
...
fn assert_export_prelude_exports<K, U, V, const D: usize>(
    ...
    assert_eq!(
        ...
        ExportPreludeVisualizationSchema
    );
    assert_eq!(
...
fn preludes_cover_bench_apis() -> Result<(), PreludeExportTestError> {
    ...
    let _query_prelude_boundary_facets: QueryBoundaryFacetsIter<'_, (), (), 3> =
        dt.boundary_facets()?;
    let (simplex_key, simplex) =
        dt.simplices()
            .next()
            .ok_or(PreludeExportTestError::MissingFixture {
                context: "simplex in a constructed tetrahedron",
            })?;
    let simplex_uuid =
        dt.simplex_uuid_from_key(simplex_key)
            .ok_or(PreludeExportTestError::MissingFixture {
                context: "UUID for a live simplex key",
            })?;
    ...
    let vertex_uuid =
        dt.vertex_uuid_from_key(vertex_key)
            .ok_or(PreludeExportTestError::MissingFixture {
                context: "UUID for a live vertex key",
            })?;
    ...
    assert_eq!(hull.facets().count(), boundary_facet_count);
...
#[test]
fn geometry_prelude_covers_typed_error_variants() -> Result<(), PreludeExportTestError> {
    let finite = FiniteCoordinateValue::try_new(1.5)?;
...
#[test]
fn geometry_prelude_covers_simplex_realization_validation() -> Result<(), PreludeExportTestError> {
    let first =
...
#[test]
fn construction_prelude_covers_explicit_tds_preflight_errors() -> Result<(), PreludeExportTestError>
{
...
fn ridge_star_prelude_vertices<const D: usize>()
-> Result<Vec<Vertex<(), D>>, PreludeExportTestError> {
    let mut vertices = Vec::with_capacity(D + 2);
    vertices.push(vertex!([-1.0; D])?);

    for axis in 0..D {
        let mut coords = [-1.0; D];
        coords[axis] = 1.0;
        vertices.push(vertex!(coords)?);
    }
    ...
    AdaptiveKernel<f64>: ExactPredicates<D>,
{
    ...
    let ridge_link: &RidgeLinkView<'_, (), (), D> =
        ridge_links
            .first()
            .ok_or(PreludeExportTestError::MissingFixture {
                context: "link for a simplex ridge",
            })?;
    ...
    let link_edge: &LiftedLinkEdge =
        link_edges
            .first()
            .ok_or(PreludeExportTestError::MissingFixture {
                context: "edge for a single-simplex ridge link",
            })?;
...
fn assert_multi_simplex_ridge_star<const D: usize>() -> Result<(), PreludeExportTestError>
where
    AdaptiveKernel<f64>: ExactPredicates<D>,
{
    let vertices = ridge_star_prelude_vertices::<D>()?;
    let dt = DelaunayTriangulation::builder(&vertices).build()?;
    let (_, simplex) = dt
        .simplices()
        .next()
        .ok_or(PreludeExportTestError::MissingFixture {
            context: "simplex in a multi-simplex ridge-star fixture",
        })?;
    let ridge =
        RidgeCandidate::<D>::try_from_vertices(simplex.vertices().iter().copied().take(D - 1))?;
    ...
    AdaptiveKernel<f64>: ExactPredicates<D>,
{
    ...
    assert_multi_simplex_ridge_star::<D>()?;
    assert_matches!(
...
#[test]
fn topology_spaces_prelude_covers_spherical_backend_api() -> Result<(), PreludeExportTestError> {
...
fn triangulation_prelude_covers_generic_layer() -> Result<(), PreludeExportTestError> {
    ...
    assert!(tri.local_facet_repair(&simplex_keys)?.is_none());
    let boundary_facet_count = tri
        .boundary_facets()?
        .try_fold(0_usize, |count, facet| facet.map(|_| count + 1))?;
    assert_eq!(boundary_facet_count, 3);
...
#[test]
fn ordering_prelude_covers_hilbert_apis() -> Result<(), PreludeExportTestError> {
    ...
    let bounds = CoordinateRange::try_new(0.0_f64, 1.0)?;
